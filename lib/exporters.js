    ...
    try {
      ...
      for (var [originalSrc, dataUri] of uniqueSrcs) {
        ...
        if (originalSrc.startsWith('data:')) {
          ...
          result = result.split(originalSrc).join(newRelativePath);
        } else {
          var escaped = originalSrc.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          var reDouble = new RegExp('(src\\s*=\\s*")' + escaped + '(")', 'g');
          var reSingle = new RegExp("(src\\s*=\\s*')" + escaped + "(')", 'g');
          result = result.replace(reDouble, '$1' + newRelativePath + '$2');
          result = result.replace(reSingle, '$1' + newRelativePath + '$2');
        }
